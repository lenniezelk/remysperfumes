...
export const fetchEditUserInitialData = createServerFn({ method: 'GET' })
    .middleware([canManageUsersMiddleware])
    .inputValidator(FetchUserByIdInput).handler(async (ctx): Promise<Result<{ user: User; roles: Array<Role> }>> => {
...
export const updateAdminUser = createServerFn({ method: 'POST' })
    .middleware([canManageUsersMiddleware])
    .inputValidator(UpdateUserData).handler(async (ctx): Promise<Result<User>> => {
        const db = dbClient();
        const data = ctx.data;
