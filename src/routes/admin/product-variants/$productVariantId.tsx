...
import { getProductVariantById } from '@/lib/server/product-variant/get'
import { updateProductVariant } from '@/lib/server/product-variant/update';
...
export const Route = createFileRoute('/admin/product-variants/$productVariantId')({
    component: RouteComponent,
    loader: async (ctx) => {
        const [productVariantResult, products] = await Promise.all([
            getProductVariantById({ data: { productVariantId: ctx.params.productVariantId } }),
            listProductsForDropdown(),
        ]);
        return { productVariantResult, products };
    },
})

function RouteComponent() {
    const { productVariantResult, products } = Route.useLoaderData();
    const productVariant = productVariantResult.status === 'SUCCESS' ? productVariantResult.data : null;
    ...
    const form = useForm({
        defaultValues: {
            ...
            product_id: productVariant ? productVariant.product_id : '',
            name: productVariant ? productVariant.name : '',
            sku: productVariant ? productVariant.sku : '',
            default_sell_price: productVariant ? (productVariant.default_sell_price || 0) : 0,
            productImage: '',
            localProductImage: null as File | null,
        },
        onSubmit: async (values) => {
            // Convert file to base64 if present
            const imageBase64 = values.value.localProductImage
                ? await fileToBase64(values.value.localProductImage)
                : '';

            const data = {
                productVariantId: values.value.productVariantId,
    ...
    if (!productVariant) {
        return <Heading level={2}>Product variant not found</Heading>;
    }
            ...
            <form
                ...
                <div>
                    <form.Field
                        ...
                        children={
                            (field) => {
                                const existingImage = productVariant?.image;
                                return (
                                    <>
                                        <label htmlFor='localProductImage' className="btn btn-neutral">
                                            {field.state.value ? 'Change Image' : (existingImage ? 'Update Image' : 'Select Product Image')}
                                        </label>
                                        <input
                                        ...
                                        {!field.state.value && existingImage && (
                                            <div className="mt-2">
