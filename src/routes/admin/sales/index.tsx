...
export const Route = createFileRoute('/admin/sales/')({
  ...
  head: () => ({
    meta: [
      { title: "Sales | Remi's Perfumes" },
    ]
  }),
  loaderDeps: ({ search: { searchQuery, sort, order, page, limit, showDeleted, from, to } }) => ({ searchQuery, sort, order, page, limit, showDeleted, from, to }),
...
function RouteComponent() {
  ...
  const data = salesQuery.data.status === 'SUCCESS' ? salesQuery.data.data.items : [];
  const total = salesQuery.data.status === 'SUCCESS' ? salesQuery.data.data.total : 0;
  const totalAmount = salesQuery.data.status === 'SUCCESS' ? (salesQuery.data.data as any).totalAmount : 0; // Cast to any because type inference might lag without strict generic updates
  const page = salesQuery.data.status === 'SUCCESS' ? salesQuery.data.data.page : 1;
  const limit = salesQuery.data.status === 'SUCCESS' ? salesQuery.data.data.limit : 10;
  const offset = salesQuery.data.status === 'SUCCESS' ? salesQuery.data.data.offset : 0;
  ...
  const handleDatePresetChange = (preset: string) => {
    ...
    if (preset === "today") {
      ...
      from = new Date(yesterday.getFullYear(), yesterday.getMonth(), yesterday.getDate()).getTime();
      to = new Date(yesterday.getFullYear(), yesterday.getMonth(), yesterday.getDate(), 23, 59, 59, 999).getTime();
    ...
    <div className="w-full overflow-x-auto">
            ...
            {datePreset === "custom" && (
              <div className="flex gap-2 items-center">
                <input
                  ...
                  value={search.from ? new Date(search.from).toISOString().split('T')[0] : ''}
                  onChange={(e) => {
                    const date = e.target.value ? new Date(e.target.value).getTime() : undefined;
                    navigate({ search: (prev) => ({ ...prev, from: date, page: 1 }) })
                  }}
                />
                ...
                <input
                  type="date"
                  className="input input-bordered input-sm"
                  value={search.to ? new Date(search.to).toISOString().split('T')[0] : ''}
                  onChange={(e) => {
                    const date = e.target.value ? new Date(e.target.value).getTime() : undefined;
                    // Set to end of day if selecting a specific date
                    const time = date ? date + 86399999 : undefined;
                    navigate({ search: (prev) => ({ ...prev, to: time, page: 1 }) })
                  }}
                />
              </div>
            )}
          ...
          <div className="flex gap-2 shrink-0">
            <div className="w-48">
              <Select
                name="sort"
                value={search.sort}
                onChange={(e) => navigate({ search: (prev) => ({ ...prev, sort: e.target.value as "date" | "total_amount" | "customer_name" }) })}
              >
            ...
            <td colSpan={table.getAllColumns().length} className="h-10">
              <div className="flex justify-between items-center px-4 mt-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  >Previous</AppLink>
                  <span className="text-sm text-gray-500">Showing {offset + 1} to {offset + data.length} of {total} sales</span>
                  <AppLink
      ...
      <dialog id="confirmDelete" className="modal" ref={deleteDialogRef}>
        <div className="modal-box">
          ...
          <p className="text-sm text-gray-500 mt-2">This will soft delete the sale. You can restore it later.</p>
